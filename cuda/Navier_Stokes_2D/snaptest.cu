...
void writeSnapshotMatrix(const std::vector<std::vector<float>>& snapshots, const std::string& filename) {
    ...
    for (size_t i = 0; i < snapshots.size(); ++i) {
        ...
        for (size_t j = 0; j < snap.size(); ++j) {
            out << snap[j];
            if (j < snap.size() - 1) out << ",";
        }
        out << "\n";
    }

    out.close();
    std::cout << "Saved " << snapshots.size() << " snapshots to file: " << filename << "\n";
}
...
int main(int argc, char** argv) {
    ...
    while (framecount < MAX_FRAMES) {
        ...
        if (framecount % SNAPSHOT_INTERVAL == 0) {
            ...
            cudaMemcpy(snapshot, dev_u, dim * dim * sizeof(Vector2f), cudaMemcpyDeviceToHost);
...
// Write the snapshots.csv file as an (N_space x M_times) matrix (i.e., column-major order)
    ...
    free(u); free(p); free(c); free(obstacleField);
